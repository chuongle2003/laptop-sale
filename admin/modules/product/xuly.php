...
if (isset($_POST['update_quantity'])) {
    ...
    if ($product_quantity < 0) {
        header('Location:../../index.php?action=product&query=tonkho&message=error');
        exit();
    }
    ...
    if ($query_update) {
        ...
        $product_status = ($product_quantity > 0) ? 1 : 0;
        $sql_update_status = "UPDATE product SET product_status = '$product_status' WHERE product_id = '$product_id'";
        mysqli_query($mysqli, $sql_update_status);

        header('Location:../../index.php?action=product&query=tonkho&message=success');
    } else {
        header('Location:../../index.php?action=product&query=tonkho&message=error');
    }
}
